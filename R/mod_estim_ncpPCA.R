#' Estimate ncp for Methyl Data
#'
#' @return list of value with and best ncp
#' @export methyl_estim_ncpPCA
methyl_estim_ncpPCA <- function(
    X, ncp.min = 0, ncp.max = 5, method = c("Regularized", "EM"), scale = TRUE,
    method.cv = c("gcv", "loo", "Kfold"), nbsim = 100, pNA = 0.05,
  ...
  if (method.cv == "gcv") {
    ...
    for (q in max(ncp.min, 1):ncp.max) {
      rec <- methyl_imputePCA(X, scale = scale, ncp = q, method = method, ...)$fittedX
        ...
        for (j in 1:ncol(X)) {
          if (!is.na(X[i, j])) {
            ...
            if (nbaxes == 0) {
              ...
              Xhat[i, j] <- methyl_imputePCA(XNA, ncp = nbaxes, threshold = threshold, method = method, scale = scale)$completeObs[i, j]
            }
          }
        }
  ...
  if (method.cv == "kfold") {
    ...
    for (sim in seq_len(nbsim)) {
      XNA <- as.matrix(X)
      XNA[sample(seq_len(nrow(XNA) * ncol(XNA)), round(pNA * nrow(XNA) * ncol(XNA), 0))] <- NA
      for (nbaxes in ncp.min:ncp.max) {
        if (nbaxes == 0) {
          Xhat <- XNA
          for (j in seq_len(ncol(X))) Xhat[, j] <- replace(XNA[, j], is.na(XNA[, j]), mean(XNA[, j], na.rm = TRUE))
        } else {
          Xhat <- methyl_imputePCA(XNA, ncp = nbaxes, threshold = threshold, method = method, scale = scale)$completeObs
        }
    ...
    resu <- rowMeans(res, na.rm = TRUE)
    names(resu) <- c(ncp.min:ncp.max)
    result <- list(
      ncp = as.integer(which.min(resu) + ncp.min - 1),
      criterion = resu,
      nbsim = nbsim
    )
  }
