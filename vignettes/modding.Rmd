...
row.weight <- rep(1, nrow(vnf0))
microbenchmark(
  imputeMCA(vnf0, ncp = 3, row.w = row.weight),
  modded_imputeMCA(vnf0, ncp = 3),
  times = 20
)

profvis::profvis(imputeMCA(vnf0, ncp = 3))
profvis::profvis(modded_imputeMCA(vnf0, ncp = 3))

waldo::compare(Zscale, modded_Zscale)
```
...
```{r}
# test <- matrix(rnorm(500*500), 500, 500)
# all(apply(test, 2 , sum) == colSums(test))
# 
# microbenchmark(
#   apply(test, 2 , sum),
#   colSums(test),
